  ...
  template: `
    <div class="chips-container">
      <div class="chip-group">
        <span class="chip-label">Búsquedas populares</span>
        <div class="chip-list">
      ...
      <div class="chip-group" *ngIf="historial().length > 0">
        <span class="chip-label">Búsquedas registradas</span>
        <div class="chip-list">
          <button class="chip recent" *ngFor="let h of historial()" (click)="alBuscar(h)">
            {{ h }}
            <button class="remove" (click)="$event.stopPropagation(); eliminarDelHistorial(h)" aria-label="Eliminar búsqueda">×</button>
          </button>
        </div>
      </div>
